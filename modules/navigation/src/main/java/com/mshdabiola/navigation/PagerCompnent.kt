...
class PagerComponent(componentContext: ComponentContext) :IPagerComponent, ComponentContext by componentContext {

    @OptIn(ExperimentalDecomposeApi::class)
    private val navigation= PagesNavigation<Config2>()
    @OptIn(ExperimentalDecomposeApi::class)
    override val stack: Value<ChildPages<*, IPagerComponent.PScreen>>
     =  childPages(
        source = navigation,
       initialPages = {
           Pages(
        ...
        when(configuration){
            ...
            is Config2.Statistic->IPagerComponent.PScreen.StatisticsRootScreen(StatisticComponent(componentContext))
            is Config2.Profile->IPagerComponent.PScreen.ProfileRootScreen(ProfileComponent(componentContext))
        }
    }
